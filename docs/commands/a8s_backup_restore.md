...
POST /api/backups/restore/{targetType}/{id}/{runId}

## Usage

```text
a8s backup restore <type> <id> <run-id> [flags]
```
...
- [a8s backup restore cancel](a8s_backup_restore_cancel.md) - POST /api/backups/restore/{targetType}/{id}/{runId}/cancel

## Backend Endpoint

- `method`: `POST`
- `endpoint`: `/api/backups/restore/{targetType}/{id}/{runId}`
