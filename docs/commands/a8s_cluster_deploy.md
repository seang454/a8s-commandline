...
- `--project-name` `string` - owning project name
- `--query` `stringArray` - add query parameter using key=value; repeatable (default `[]`)
- `--release-name` `string` - cluster deployment release name
...
- `method`: `POST`
- `endpoint`: `/api/namespaces/{namespace}/cluster-deployments`
